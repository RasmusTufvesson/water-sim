...
def scale(pos: float) -> int:
    return min(D_SIZE-1,math.floor(pos//8))
...
class Point:
    ...
    def update(self, distribution: numpy.ndarray, delta: float):
            ...
            for rel_y in (1, 0, -1):
                if rel_x == 0 and rel_y == 0:
                    ...
                    if score < min_score:
                        ...
                        min_score = score
        if zero > min_score:
            add_vel += direction * REPEL * math.log10(zero - min_score)
        add_vel *= delta * 0.5
        self.vel += add_vel
...
points = [Point(Vector2(x,y)) for x in range(10, 496, 2) for y in range(250, 270, 5)]
...
while on:
    ...
    for p in points:
        p.update(distribution, delta)
        # pygame.draw.circle(screen, (30,30,150), p.pos, 2)
    for x in range(D_SIZE):
        for y in range(D_SIZE):
            v = distribution[x, y]
            if v > 0:
                pygame.draw.rect(screen, (min(5*v,255),min(5*v,255),min(30*v,255)), (x * REV_SCALE, y * REV_SCALE, REV_SCALE, REV_SCALE))
